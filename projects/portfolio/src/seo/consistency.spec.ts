...
describe("the edge function's copy of the metadata", () => {
  // Deno cannot load the app's JSON or its extensionless modules, so the edge
  // function reads a generated .ts copy instead (see netlify/edge-functions/lib).
  // Two files holding the same data is exactly the drift this directory exists to
  // prevent, so the copy is checked rather than trusted.
  const generated = read('netlify/edge-functions/lib/site-metadata.generated.ts');
  const literal = generated.slice(generated.indexOf('{'), generated.lastIndexOf('}') + 1);

  it('is byte-for-byte the same data as site-metadata.json', () => {
    expect(JSON.parse(literal)).toEqual(siteMetadata);
  });

  it('is current — rerun `node scripts/generate-seo.mjs` if this fails', () => {
    expect(generated).toContain('Generated by scripts/generate-seo.mjs');
    expect(JSON.parse(literal).pages).toHaveLength(siteMetadata.pages.length);
  });
});
